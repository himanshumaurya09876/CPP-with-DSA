...
bool isSafe(char maze[][1001],int n,int m,int i,int j)
{
...
bool findPathForRat(char maze[][1001],bool sol[][1001],int n,int m,int i=0,int j=0)
{
	if(i==n-1 && j==m-1)
	{
	...
	{
		sol[i][j]=1;
		bool movedRat=findPathForRat(maze,sol,n,m,i,j+1);
		if(movedRat)
		{
			return true;
		}
		movedRat=findPathForRat(maze,sol,n,m,i+1,j);
		if(movedRat)
		{
			return true;
		}
...
int main(int argc, char const *argv[])
{
	...
	if(findPathForRat(maze,sol,n,m))
	{
		for(int i=0;i<n;i++)
	 	{
			for(int j=0;j<m;j++)
